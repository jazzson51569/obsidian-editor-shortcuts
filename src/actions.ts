...
export const moveLineUp = (
  ...
  if (line === 0) {
    return {
      changes: [],
      newSelection: { from: selection.head },
    };
  }

  const currentLineContent = editor.getLine(line);
  const prevLineContent = editor.getLine(line - 1);
  const currentLineStart = getLineStartPos(line);
  const currentLineEnd = getLineEndPos(line, editor);
  const prevLineStart = getLineStartPos(line - 1);

  // 交换两行内容：删除当前行，然后在上一行位置插入当前行内容
  const changes: EditorChange[] = [
    {
      from: prevLineStart,
      to: currentLineEnd,
...
export const moveLineDown = (
  ...
  const currentLineStart = getLineStartPos(line);
  const nextLineEnd = getLineEndPos(line + 1, editor);

  // 交换两行内容：删除当前行和下一行，然后在下一行位置插入当前行内容
  const changes: EditorChange[] = [
    {
 ...
 * @param editor - Obsidian 编辑器实例
 */
export const moveCursorToFirstLine = (editor: Editor) => {
 ...
 * @param editor - Obsidian 编辑器实例
 */
export const moveCursorToLastLine = (editor: Editor) => {
  const lastLine = editor.lastLine();
